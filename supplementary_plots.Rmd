...
```
```{r}
temp_skill_RW <- temperature_skill_RW |>
  group_by(model_id) |>
  summarise(mean_crps = mean(skill_crps, na.rm = T)) |>
  ungroup() |>
  filter(mean_crps < 0) |>
...
ggsave(skill_horizon_RW, filename = 'Plots/FigureSnew1.png', height = 10, width = 13, units = 'cm')
```
...
ggarrange(null_scores_T, null_scores_DO, common.legend = T) %>% 
  ggsave(filename = 'Plots/Figure_Snew2.png', height = 10, width = 18, units = 'cm')
```
