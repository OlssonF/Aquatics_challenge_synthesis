...
```{r}
# total forecasts
distinct_forecasts <- arrow::open_dataset("./scores_2023") |> 
...
ggpubr::ggarrange(top10_temp, all_temp, nrow = 1, align = 'hv',
  ...
  ggsave(filename = './Plots/Figure2.png', width = 25, height = 11, units = 'cm')
...
all_chla <- chla_skill |> 
  ...
  left_join(model_meta) |> 
  mutate(manuscript_name = fct_reorder(manuscript_name, mean_crps),
         ast = ifelse(mean_crps < -100, '>>', ''),
         uses_NOAA = ifelse(uses_NOAA == 1, 'yes', 'no'), 
         uses_AT = as.factor(str_detect(NOAA_var, 'air_temperature')),
         mean_crps = ifelse(mean_crps < -100, -100, mean_crps)) |> 
  ggplot(aes(x=mean_crps, y = manuscript_name, fill = model_type)) +
  annotate('rect', xmax = Inf, xmin = -Inf, ymin = 20.5, ymax =  Inf, fill = 'grey', alpha = 0.2) + 
  geom_bar_pattern(aes(pattern = uses_AT),# Different pattern for each group
                   stat = "identity",
                   alpha = 0.8, 
                   colour = 'black',
                   pattern_fill = "black",
                   ...
                   pattern_alpha = 1) + 
  scale_y_reordered() +
  labs(y='Model', x = 'Mean relative skill (ug/L)', subtitle = 'Chlorophyll a') +
  geom_vline(xintercept = 0) +
  scale_fill_manual(values = cols_modeltype) +
  scale_pattern_manual(values=c('none', 'stripe'), name = 'Uses air temperature as a covariate?') +
  scale_pattern_type_manual(values=c(NA, NA), name = 'Uses air temperature as a covariate?') +
  ...
  geom_text(aes(y=manuscript_name, x= 0.75, label = ast), inherit.aes = F, )

ggpubr::ggarrange(top10_temp, all_temp, all_DO, all_chla, nrow = 2, ncol= 2, align = 'hv',
                  common.legend = T,
                  legend = 'top', labels = 'auto') |> 
  ggsave(filename = 'Plots/Figure2.png', height = 20, width = 25, units = 'cm')
